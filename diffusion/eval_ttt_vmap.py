...
def _ttt_cfg_from_config(
    ...
    *,
    ...
    return TTTConfig(
        ...
        task_chunk_size=max(1, int(cfg.ttt.task_chunk_size)),
...
def _build_context_only_batch_for_tasks_with_rngs(
    tasks: List[Dict[str, Any]],
    *,
    device: torch.device,
    ...
    if len(tasks) != len(rngs):
        raise ValueError("tasks and rngs must have the same length.")

    context_sequences: List[torch.Tensor] = []
    context_lengths: List[int] = []

    for task, task_rng in zip(tasks, rngs):
        selected_context_episodes = _sample_context_subset(
            task["context_episodes"],
            num_context_episodes=num_context_episodes,
            rng=task_rng,
...
def _stack_task_support_batches(
    ...
    *,
    ...
    support_batch_size = int(task_batches[0]["actions"].shape[0])
    horizon = int(task_batches[0]["actions"].shape[1])
    feature_dim = int(task_batches[0]["actions"].shape[2])
    dtype = task_batches[0]["actions"].dtype

    for task_batch in task_batches[1:]:
        if int(task_batch["actions"].shape[0]) != support_batch_size:
            raise ValueError("All task support batches must share the same batch size.")

    max_history_len = max(int(task_batch["history"].shape[1]) for task_batch in task_batches)
    max_context_len = max(int(task_batch["context"].shape[1]) for task_batch in task_batches)
    max_query_mask_len = max(int(task_batch["query_mask"].shape[1]) for task_batch in task_batches)

    history = torch.zeros(
        (num_tasks, support_batch_size, max_history_len, feature_dim),
        dtype=dtype,
        device=device,
    )
    actions = torch.zeros(
        (num_tasks, support_batch_size, horizon, feature_dim),
        dtype=dtype,
        device=device,
    )
    context = torch.zeros(
        (num_tasks, support_batch_size, max_context_len, feature_dim),
        dtype=dtype,
        device=device,
    )
    query_mask = torch.zeros(
        (num_tasks, support_batch_size, max_query_mask_len),
        dtype=torch.bool,
        device=device,
    )
    context_mask = torch.zeros(
        (num_tasks, support_batch_size, max_context_len),
        dtype=torch.bool,
        device=device,
    )

    noise = None
    if "noise" in task_batches[0]:
        noise = torch.zeros(
            (num_tasks, support_batch_size, horizon, feature_dim),
            dtype=torch.float32,
            device=device,
    ...
    timesteps = None
    if "timesteps" in task_batches[0]:
        timesteps = torch.zeros(
            (num_tasks, support_batch_size),
            dtype=torch.long,
            device=device,
        )

    for task_idx, task_batch in enumerate(task_batches):
        history_len = int(task_batch["history"].shape[1])
        context_len = int(task_batch["context"].shape[1])
        query_mask_len = int(task_batch["query_mask"].shape[1])

        history[task_idx, :, -history_len:] = task_batch["history"]
        actions[task_idx] = task_batch["actions"]
        context[task_idx, :, -context_len:] = task_batch["context"]
        query_mask[task_idx, :, -query_mask_len:] = task_batch["query_mask"]
        context_mask[task_idx, :, -context_len:] = task_batch["context_mask"]

        if noise is not None and "noise" in task_batch:
            noise[task_idx] = task_batch["noise"]
        if timesteps is not None and "timesteps" in task_batch:
            timesteps[task_idx] = task_batch["timesteps"]
...
def _stack_base_fast_params(
    ...
    *,
    ...
    return tuple(
        ...
        .detach()
        .unsqueeze(0)
        .expand((num_tasks, *params[name].shape))
        .clone()
        for name in fast_names
    )


def _clip_task_grads(
    grads: tuple[torch.Tensor, ...],
    max_norm: float,
) -> tuple[torch.Tensor, ...]:
    if max_norm <= 0 or not grads:
        return grads
    per_task_sq_norm = None
    for grad_tensor in grads:
        grad_sq = grad_tensor.reshape(grad_tensor.shape[0], -1).pow(2).sum(dim=1)
        per_task_sq_norm = grad_sq if per_task_sq_norm is None else per_task_sq_norm + grad_sq
    assert per_task_sq_norm is not None
    per_task_norm = per_task_sq_norm.sqrt()
    scale = torch.clamp(max_norm / (per_task_norm + 1e-6), max=1.0)
    return tuple(
        grad_tensor * scale.view(scale.shape[0], *([1] * (grad_tensor.ndim - 1)))
        for grad_tensor in grads
    )


class _PredictDenoisingStepModule(nn.Module):
    def __init__(self, policy: MAMLDiTEncDecDiffusionPolicy) -> None:
        super().__init__()
        self.policy = policy
    ...
    def forward(
        ...
        *,
        ...
        return self.policy.predict_denoising_step(
            context=context,
            context_mask=context_mask,
            history=history,
...
def _adapt_fast_params_for_tasks_vmap(
    model: nn.Module,
    tasks: List[Dict[str, Any]],
    *,
    fast_names: List[str],
    cfg: TTTConfig,
    horizon: int,
    task_rngs: List[torch.Generator],
) -> tuple[torch.Tensor, ...]:
    if len(tasks) != len(task_rngs):
        raise ValueError("tasks and task_rngs must have the same length.")
    if not fast_names:
        return ()
    ...
    shared_params = {k: v for k, v in params.items() if k not in fast_names}
    adapted_fast_params = _stack_base_fast_params(
        model,
        fast_names=fast_names,
        num_tasks=len(tasks),
    )

    shared_noise: List[Optional[torch.Tensor]] = [None] * len(tasks)
    shared_timesteps: List[Optional[torch.Tensor]] = [None] * len(tasks)

    def _single_task_support_loss(
        task_fast_params: tuple[torch.Tensor, ...],
        task_batch: Dict[str, torch.Tensor],
    ) -> torch.Tensor:
        full_params = dict(shared_params)
        for name, tensor in zip(fast_names, task_fast_params):
            full_params[name] = tensor
        return functional_call(model, (full_params, buffers), (task_batch,))

    task_grad_fn = grad(_single_task_support_loss)

    for task_step in range(cfg.inner_steps):
        del task_step
        task_batches: List[Dict[str, torch.Tensor]] = []
        for task_idx, (task, task_rng) in enumerate(zip(tasks, task_rngs)):
            loo_indices = _sample_loo_indices(
                len(task["context_episodes"]),
                num_loo_per_task=cfg.num_loo_per_task,
                device=device,
                rng=task_rng,
            )
            if (
                model.objective_type == "diffusion"
                and cfg.reuse_diffusion_noise
                and (shared_noise[task_idx] is None or shared_timesteps[task_idx] is None)
            ):
                feature_dim = int(task["query_episode"].shape[-1])
                shared_noise[task_idx] = torch.randn(
                    (1, horizon, feature_dim),
                    device=device,
                    dtype=torch.float32,
                )
                shared_timesteps[task_idx] = torch.randint(
                    0,
            ...
            task_batches.append(
                build_support_batch_loo(
                    ...
                    horizon=horizon,
                    device=device,
                    noise=shared_noise[task_idx] if cfg.reuse_diffusion_noise else None,
                    timesteps=shared_timesteps[task_idx] if cfg.reuse_diffusion_noise else None,
                    rng=task_rng,
                )
            )

        stacked_task_batch = _stack_task_support_batches(task_batches, device=device)
        task_grads = vmap(
            task_grad_fn,
            in_dims=(0, 0),
            randomness="different",
        )(adapted_fast_params, stacked_task_batch)
        task_grads = _clip_task_grads(task_grads, cfg.max_grad_norm)
        adapted_fast_params = tuple(
            param - cfg.inner_lr * grad_tensor
            for param, grad_tensor in zip(adapted_fast_params, task_grads)
        )

    return adapted_fast_params


@torch.no_grad()
def _sample_quickdraw_tokens_encoder_decoder_vmap(
    *,
    policy: MAMLDiTEncDecDiffusionPolicy,
    adapted_fast_params: tuple[torch.Tensor, ...],
    fast_names: List[str],
    max_tokens: int,
    ...
    if not fast_names:
        return sample_quickdraw_tokens_encoder_decoder(
            policy=policy,
    ...
    demos = {key: value.to(device) for key, value in demos.items()}
    context = demos["context"]
    context_mask = demos["context_mask"]
    batch_size = int(context.shape[0])

    if adapted_fast_params[0].shape[0] != batch_size:
        raise ValueError(
            "adapted_fast_params batch dimension must match demos batch size."
        )
    ...
    horizon = int(policy.cfg.horizon)
    action_dim = int(policy.cfg.action_dim)
    ...
    history_mask = torch.cat(
        [
            history_mask,
            torch.ones((batch_size, horizon), device=device, dtype=torch.bool),
        ],
        dim=1,
    )

    wrapper = _PredictDenoisingStepModule(policy).to(device)
    wrapper_buffers = {f"policy.{name}": buffer for name, buffer in policy.named_buffers()}
    wrapper_shared_params = {
        f"policy.{name}": param
        for name, param in policy.named_parameters()
        if name not in fast_names
    }
    wrapper_fast_names = [f"policy.{name}" for name in fast_names]

    def _predict_chunk_for_task(
        task_fast_params: tuple[torch.Tensor, ...],
        task_context: torch.Tensor,
        task_context_mask: torch.Tensor,
        task_history: torch.Tensor,
        task_history_mask: torch.Tensor,
        task_future_slots: torch.Tensor,
        task_timesteps: torch.Tensor,
    ) -> torch.Tensor:
        full_params = dict(wrapper_shared_params)
        for name, tensor in zip(wrapper_fast_names, task_fast_params):
            full_params[name] = tensor
        return functional_call(
            wrapper,
            (full_params, wrapper_buffers),
            kwargs={
                "context": task_context.unsqueeze(0),
                "context_mask": task_context_mask.unsqueeze(0),
                "history": task_history.unsqueeze(0),
                "history_mask": task_history_mask.unsqueeze(0),
                "future_slots": task_future_slots.unsqueeze(0),
                "timesteps": task_timesteps.unsqueeze(0),
            },
        )[0]

    predict_vmap = vmap(
        _predict_chunk_for_task,
    ...
    for _ in range(max_chunks):
        if policy.objective_type == "direct_regression":
            ...
            policy.scheduler.set_timesteps(num_inference_steps, device=device)
            for timestep in policy.scheduler.timesteps:
        ...
        generated_chunks.append(actions)
        history = torch.cat([history, actions], dim=1)
        history_mask = torch.cat(
            [
                history_mask,
                torch.ones(actions.shape[:2], device=device, dtype=torch.bool),
            ],
            dim=1,
        )

    generated = torch.cat(generated_chunks, dim=1)
    return clean_sketches_encoder_decoder(generated)
    ...
    *,
    ...
    with tqdm(
        ...
        while len(generated) < int(cfg.eval.fid.num_samples):
            ...
            task_chunk_size = max(1, int(ttt_cfg.task_chunk_size))
            for chunk_start in range(0, len(tasks), task_chunk_size):
                remaining = int(cfg.eval.fid.num_samples) - len(generated)
                if remaining <= 0:
                    break
                chunk_tasks = tasks[chunk_start : chunk_start + min(task_chunk_size, remaining)]
                global_chunk_offset = len(generated)
                task_rngs: List[torch.Generator] = []
                for task_offset in range(len(chunk_tasks)):
                    task_seed = int(cfg.eval.seed) + global_chunk_offset + task_offset
                    task_rng = torch.Generator(device=device)
                    task_rng.manual_seed(task_seed)
                    task_rngs.append(task_rng)

                policy.train()
                with _maml_attention_ctx(ttt_cfg, device), torch.enable_grad():
                    adapted_fast_params = _adapt_fast_params_for_tasks_vmap(
                        policy,
                        chunk_tasks,
                        fast_names=fast_names,
                        cfg=ttt_cfg,
                ...
                context_batch = _build_context_only_batch_for_tasks_with_rngs(
                    chunk_tasks,
                    device=device,
                    rngs=task_rngs,
                    num_context_episodes=ttt_cfg.outer_context_size,
                ...
                sample_generator = torch.Generator(device=device)
                sample_generator.manual_seed(int(cfg.eval.seed) + global_chunk_offset)
                samples = _sample_quickdraw_tokens_encoder_decoder_vmap(
                    policy=policy,
                    adapted_fast_params=adapted_fast_params,
                    fast_names=fast_names,
                    max_tokens=max_tokens,
                    demos=context_batch,
                    generator=sample_generator,
                ...
                gt_batch = _query_sketches_from_tasks(chunk_tasks)
                take = min(remaining, len(samples), len(gt_batch))
                generated.extend(samples[:take])
                gt_queries.extend(gt_batch[:take])
                progress.update(take)
...
def _write_eval_summary(
    *,
    ...
    summary = {
        ...
        "task_chunk_size": int(ttt_cfg.task_chunk_size),
...
def main(_) -> None:
    ...
    print(
        ...
        f"task_chunk_size={ttt_cfg.task_chunk_size}, "
