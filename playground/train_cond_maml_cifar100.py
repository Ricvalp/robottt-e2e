...
def build_datasets(cfg: ConfigDict):
    ...
    # Get all class labels (CIFAR-100 has 100 classes)
...
def sample_class_pair_batches(dataset, indices_by_class, cls: int, batch_size_x: int, batch_size_cond: int, device: torch.device):
    """
    Returns two batches of the same class but (ideally) different instances:
...
def train(cfg: ConfigDict) -> None:
    ...
    for epoch in range(1, cfg.training.epochs + 1):
        ...
        for _ in pbar:
            ...
            imgs, cond_imgs = sample_class_pair_batches(
                dataset,
                train_indices_by_class,
                cls,
                cfg.data.batch_size,
        ...
        if epoch % cfg.training.sample_every_epochs == 0:
            ...
            val_imgs, val_cond = sample_class_pair_batches(
                dataset, val_indices_by_class, cls, cfg.data.batch_size, cfg.data.cond_batch_size, device
            )
            ...
            tr_imgs, tr_cond = sample_class_pair_batches(
                dataset, train_indices_by_class, train_cls, cfg.data.batch_size, cfg.data.cond_batch_size, device
            )
...
_CONFIG = config_flags.DEFINE_config_file(
    "config",
    default="playground/configs/train_cond_maml_cifar100.py",
