#!/usr/bin/env python3
"""
CelebA Classifier Training with DDP for FID computation.
Trains a multi-label classifier on 40 binary attributes.

Launch with:
    torchrun --nproc_per_node=N train_classifier_celeba.py --classifier_config=configs/train_classifier_celeba.py
"""
...
def build_loaders(cfg: ConfigDict, world_size: int, rank: int) -> Tuple[DataLoader, DataLoader, DistributedSampler]:
    ...
    train_ds = datasets.CelebA(cfg.data.root, split="train", download=cfg.data.download, transform=transform)
    test_ds = datasets.CelebA(cfg.data.root, split="valid", download=cfg.data.download, transform=transform)
...
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device) -> float:
    """Compute average attribute prediction accuracy."""
    model.eval()
    correct = 0
    total = 0
    with torch.no_grad():
        for x, attrs in tqdm(loader, desc="Evaluating", leave=False) if is_main_process() else loader:
            x = x.to(device)
...
def train(cfg: ConfigDict) -> None:
    ...
    model = SmallResNet(
        ...
        num_classes=cfg.model.num_attributes,  # 40 binary attributes
    ...
    for epoch in range(1, cfg.training.epochs + 1):
        ...
        for x, attrs in pbar:
            x = x.to(device)
            attrs = attrs.float().to(device)  # Convert to float for BCE loss
        ...
        if is_main_process():
            ...
            if test_acc > best_acc:
                ...
                ckpt_path = ckpt_dir / "celeba_classifier.pt"
...
_CONFIG = config_flags.DEFINE_config_file(
    "classifier_config",
    default="playground/configs/train_classifier_celeba.py",
